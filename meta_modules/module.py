...
class MetaModule(nn.Module):
    """
    Base class for PyTorch meta-learning meta_modules. These meta_modules accept an
    additional argument `params` in their `forward` method.

    Notes
    -----
    Objects inherited from `MetaModule` are fully compatible with PyTorch
    meta_modules from `torch.nn.Module`. The argument `params` is a dictionary of
